...
static struct vsfmount *aufs_mount;
...
static int aufs_create_by_name(const char *name, mode_t mode, struct dentry *parent, struct dentry **dentry) {
    ...
    if (!parent) {
        printf("An! can not find a parent!\n");
        return -EFAULT;
    }
...
static dentry *aufs_create_flie(const char *name, mode_t mode, void *data, struct file_operations *fops) {
